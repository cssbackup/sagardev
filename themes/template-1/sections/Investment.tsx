          ...
          {items.map((item) => (
            ...
            >
              <div className="relative aspect-4/3 overflow-hidden bg-[#f3efe8]">
                ...
                />
                {item.yieldLabel && (
                  <span className="absolute left-3 top-3 rounded-full bg-white/95 px-2.5 py-1 text-[10px] font-semibold text-[#141414]">
                    {item.yieldLabel}
                  </span>
                )}
              </div>
              <div className="flex flex-1 flex-col p-4">
                ...
                <h3 className="mt-1.5 text-base font-semibold text-[#141414]">{item.title}</h3>
                <p className="mt-1.5 flex-1 text-xs leading-relaxed text-[#141414]/65 line-clamp-3">{item.desc}</p>
                <span className="mt-3 inline-flex items-center gap-2 text-xs font-medium text-[#141414]">
                  View opportunity
